        ...
        <div class="modal-back">
            <div class="type">
                <div style="width: 50%;">
                    ...
                    <input class="input" v-model="spaceName" type="text" placeholder="공간 이름 입력">
                </div>
                <div style="width: 50%;">
                    <div style="padding: 0 1em 0.5em 0">
                        이미지(최대 5개)
                    </div>
                    <input class="file" id="file" type="file" accept="image/*" @change="addImage" />
                </div>
            </div>
            <div class="type">
                <div style="width: 50%;">
                    ...
                    <select class="select" v-model="type">
                        <option selected value="오피스">오피스</option>
                ...
                <div style="width: 50%;">
                    ...
                    <input v-if="isOffice" style="width: 80px;padding: 0.5em" v-model="officePrice" type="text" />
            ...
            <div class="time">
                ...
                <div style="width: 50%">
                    ...
                    <select :disabled="endCleanDisabled" class="select" v-model="endCleanTime">
                        <option v-for="time in 24" :key="time">{{time - 1}}:00</option>
                    </select>
                </div>
            </div>
            <div class="button">
                ...
                <button class="reg" @click="registerSpace">등록 완료</button>
            </div>
        </div>
    </div>
</template>
...
export default {
    setup() {
        ...
        const endCleanDisabled = ref(true);
        const imgs = ref([]);
        ...
        const addImage = (event) => {
            if(imgs.value.length === 5) {
                ...
                imgs.value.push(event.target.files[0]);
                alert(imgs.value.length + '번째 이미지가 업로드 되었습니다');
            }
        }
        ...
        const registerSpace = async () => {
            if(spaceName.value.length < 3) {
            ...
            } else if(imgs.value.length === 0) {
                alert('최소 하나의 이미지를 업로드하세요');
            } else if(type.value === '') {
                alert('공간 타입을 선택하세요');
                ...
                form.append('closeTime', endTime.value);
                form.append('breakOpen', startCleanTime.value);
                form.append('breakClose', endCleanTime.value);
